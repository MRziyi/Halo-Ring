...
1. **[Doc/05-handoff.md](Doc/05-handoff.md)** — the canonical "where we are, what's left, where
   to look" document. The §2 priority-ordered TODO list is the authoritative roadmap.
2. **[Doc/](Doc/)** — the 14-doc design specification. Doc/03 (architecture) + Doc/04
...
- **No persistent wakelock**, no continuous BLE scanning, no continuous health-data streaming
  (Doc/06 explains why; Doc/08 §5 is the "explicitly out of scope" list).
- **One green accent, pure black background, no per-profile color theming** (Doc/05 §2).
...
- Material 3 widget defaults in the UI — Doc/05 §1 spells out why we don't want them.
...
- Comments: prefer "why this is here" over "what this does". The architecture doc (Doc/03) is
